...
def publishReleaseCandidate(slavePrefix, dockerHubCredentialsId) {
    stage("Publish Release Candidate") {
        node ("${slavePrefix}dev") {
            withCredentials([[$class: "UsernamePasswordMultiBinding", credentialsId: dockerHubCredentialsId, passwordVariable: "DOCKER_HUB_PASSWORD", usernameVariable: "DOCKER_HUB_PASSWORD"]]) {
                wrap([$class: "AnsiColorBuildWrapper"]) {
                    sh "docker login -u '\${DOCKER_HUB_PASSWORD}' -p '\${DOCKER_HUB_PASSWORD}'; docker push mergermarket/cdflow-commands:${commitObject.GIT_COMMIT}"
                }
            }
        }
    }
...
def publishRelease(slavePrefix, dockerHubCredentialsId) {
    stage("Publish Release") {
        node ("${slavePrefix}dev") {
            withCredentials([[$class: "UsernamePasswordMultiBinding", credentialsId: dockerHubCredentialsId, passwordVariable: "DOCKER_HUB_PASSWORD", usernameVariable: "DOCKER_HUB_PASSWORD"]]) {
                wrap([$class: "AnsiColorBuildWrapper"]) {
                    sh "docker login -u '\${DOCKER_HUB_PASSWORD}' -p '\${DOCKER_HUB_PASSWORD}'; docker tag mergermarket/cdflow-commands:latest; docker push mergermarket/cdflow-commands:latest"
                }
            }
        }
    }
